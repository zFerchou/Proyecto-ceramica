...
export default function InventoryPage({ onClose }) {
  ...
  // Función para imprimir etiquetas (solo nombre, código de barras y precio) optimizada para más etiquetas por página
  const handlePrintLabels = (producto) => {
    const quantity = parseInt(labelQuantities[producto.id_producto]) || 1;
    const validQuantity = Math.max(1, Math.min(300, quantity)); // Permitimos hasta 300 pequeñas etiquetas
    ...
    setMessage(`🖨️ Imprimiendo ${validQuantity} etiquetas optimizadas para ${producto.nombre}`);
    closeActionsModal();
  };
