...
export const crearProducto = async (req, res) => {
  ...
  try {
    ...
    const codigo = uuidv4();
    await client.query(`INSERT INTO codigo_barras (codigo, id_producto) VALUES ($1, $2)`, [codigo, id_producto]);

    await client.query('COMMIT');
    return res.status(201).json({ message: 'Producto registrado exitosamente', id_producto, codigo_barras: codigo });
  } catch (error) {
    await client.query('ROLLBACK').catch(() => {});
    console.error('crearProducto error:', error.message, error.code ? `code=${error.code}` : '');
    if (error && error.code) {
      switch (error.code) {
        case '23503': return res.status(400).json({ error: 'Referential integrity error' });
        case '23505': return res.status(409).json({ error: 'Duplicate entry' });
        case '42P01': return res.status(500).json({ error: 'Database table not found' });
        default: return res.status(500).json({ error: 'Database error', code: error.code });
      }
    }
...
export const listarProductos = async (req, res) => {
  try {
    ...
    console.error('listarProductos error:', error.message, error.code ? `code=${error.code}` : '');
    return res.status(500).json({ error: 'Error al listar productos', code: error.code });
  }
};
...
export const actualizarStock = async (req, res) => {
  ...
  if (!id_producto || isNaN(Number(id_producto))) return res.status(400).json({ error: 'id_producto must be numeric' });
  if (cantidad === undefined || !Number.isInteger(cantidad) || cantidad <= 0) return res.status(400).json({ error: 'cantidad must be positive integer' });
  ...
  try {
    ...
    console.error('actualizarStock error:', error.message, error.code ? `code=${error.code}` : '');
...
export const actualizarStockPorCodigo = async (req, res) => {
  ...
  if (!codigo || typeof codigo !== 'string') return res.status(400).json({ error: 'codigo must be string' });
  if (!cantidad || !Number.isInteger(cantidad) || cantidad <= 0) return res.status(400).json({ error: 'cantidad must be positive integer' });
  ...
  try {
    ...
    if (prod.rowCount === 0) return res.status(404).json({ error: 'Producto no encontrado para el codigo' });
    ...
    console.error('actualizarStockPorCodigo error:', error.message, error.code ? `code=${error.code}` : '');
    return res.status(500).json({ error: 'Database error', code: error.code });
  }
};
...
export const eliminarProducto = async (req, res) => {
  ...
  try {
    ...
    await client.query('ROLLBACK').catch(() => {});
    console.error('eliminarProducto error:', error.message, error.code ? `code=${error.code}` : '');
    return res.status(500).json({ error: 'Database error', code: error.code });
  } finally {
    client.release();
  }
...
export const actualizarDetalles = async (req, res) => {
  ...
  if (precio !== undefined) { updates.push(`precio = $${idx++}`); values.push(precio); }
  if (id_categoria !== undefined) { updates.push(`id_categoria = $${idx++}`); values.push(id_categoria); }
  ...
  try {
    ...
    if (nombre !== undefined) {
      ...
      if (exists.rowCount > 0) { await client.query('ROLLBACK'); return res.status(409).json({ error: 'Otro producto con ese nombre ya existe' }); }
    }
    ...
    if (result.rowCount === 0) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Producto no encontrado' }); }

    await client.query('COMMIT');
    return res.json({ message: 'Producto actualizado correctamente', producto: result.rows[0] });
  } catch (error) {
    await client.query('ROLLBACK').catch(() => {});
    console.error('actualizarDetalles error:', error.message, error.code ? `code=${error.code}` : '');
    return res.status(500).json({ error: 'Database error', code: error.code });
  } finally {
    client.release();
  }
